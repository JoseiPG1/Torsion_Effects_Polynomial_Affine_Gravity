...
Einstein's theory of General Relativity, it is a gravitational theory where the gravitational interactions are solely mediated by one 
...
with the symmetries of the observable universe (cosmological principle), it is possible to use Einstein's Field Equations \cite{Einstein_GR_Feqs} 
to describe the universe, however the above description leads to a static universe.

With the introducing the cosmological constant (see Ref. \cite{Einstein_Cosmological_Constant}) it is possible to obtain non-trivial dynamics to 
...
of the current state of the universe. In order to have a more accurate description of the observable universe, and to have non-trivial solutions, 
...
epochs, \textit{starting} from a radiation era, to a matter era and finally to an accelerated expansion due to dark matter presence. This is the 
universe describe by Friedmann–Lemaitre–Robertson–Walker (FRWL), see Refs. \cite{Frie,Friedmann,Lema,Lema_Expansion,Robertson_1,Robertson_2,Robertson_3}, 
...
predictions with the observational data, to see a full review on its empirical evidence, see Ref. \cite{Will_2014}. Perhaps, one of its more
...
Despite its success, there are still non-trivial problems related to its description of the geometry of space-time, specifically problems related 
...
its features, and how its possible to still have two different emergent-metrics structures despite being a purely affine model. A complete scan of cosmological
solutions to the field equations are presented in Section \ref{sec:solutions}, using the \textit{logical scheme} strategy. In Section \ref{sec:analysis}, it is
presented a detailed analysis and discussion of the cosmological solutions, with its physical interpretation. Final remarks are presented in Section \ref{sec:final_remarks}. 
For the sake of completeness we include to two appendixes. In Appendix \ref{sec:Appendix_I} there is a detailed description on how to build the cosmological ansatz for the irreducible fields
...
its not satisfied in the affine geometry, meaning $\nabla \mathcal{R}$ does not vanishes, and as a consequence of this, there are non-metricity effects
...
Moreover, just like the Ricci tensor, its covariant derivative $\nabla \mathcal{P}$ does not vanishes, and, therefore there are non-metricity effects
...
allows for an overdetermined system of differential equations, see eqs. \eqref{Feq_1} to \eqref{Feq_5}, whereas without torsion effects, 
the field equations are reduced to the \textit{harmonic curvature} $\nabla_\alpha \mathcal{R}_{\beta\gamma}{}^{\alpha}{}_{\delta} = 0$, which 
