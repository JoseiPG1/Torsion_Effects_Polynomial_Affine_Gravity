...
of the geometry of space-time, specifically problems related to a quantization of gravity \cite{PhysRev.160.1113,PhysRev.162.1195,PhysRevD.10.401,PhysRevD.10.411} and 
...
that allows to describe a universe without a singularity. Moreover, due to quantum corrections to general relativity, lead to an new contribution 
...
Because the choice on function $f(\mathcal{R})$ its an arbitrary one, there is an abundance of cosmological models, however, one could even 
...
the type of modify models known as \textit{metric-affine} models, and its formalism it is known as \textit{Palatini} formalism. There is an 
...
These type of affine formulations models of gravity, use the square-root of the Ricci tensor instead of the metric tensor to define the invariant element
...
which uses the Einstein-Hillbert action to define gravitational interactions, with a non-symmetric connection by introducing non-trivial 
torsion effects. Moreover, the inclusion of extra dimensions where first proposed and studied by T. Kaluza and O. 
Klein in Refs. \cite{Kaluza_Klein}, in particular, uses a fifth dimension which allows to unify the gravitational with an 
...
There are many more features, for example: i) all coupling constant are dimensionless,\footnote{This also points in the direction of a conformal symmetry, but this is highly speculative.} this its an indication that its power-counting renormalizable, which is a necessary
...
\end{dmath}
where $\mathcal{F}(g,\dot{g},h,\psi,\eta) = B_3\left(\dot{g} + gh + 2\kappa\right) - 2B_4\left(\dot{g} - gh\right) + 2D_6\eta g - 2F_3\psi^2$ and 
$\mathcal{G}(h,\psi, \dot{\psi}, \eta) = B_3\eta\psi -2B_4\eta\psi + C_1\left(\dot{\psi} - 2h\psi\right)$. Thus, from eqs. \eqref{Feq_1_1} and \eqref{Feq_2_2}
...
The only non-trivial solutions can be found in branch \eqref{branch_1}, thus, this is the only branch which 
will be studied in the following subsection. In order to simplify our calculations, we are going to assume 
the values of the integration constant $\phi_0 \to 1$, $\phi_1 \to 0$ and $g_0 \to 0$. Thus, the affine functions 
are simplify up to
\begin{align}
...
its not satisfied in the affine geometry, meaning $\nabla \mathcal{R}$ does not vanishes, and as a consequence of this, there are metricity effects
$\mathcal{Q}^{\mathcal{R}}_{\mu\nu\lambda}$ coming from the non-trivial effects related to the torsion tensor.

Additionally, it is also possible to compute Poplwaski's metric-torsion tensor which was defined in eq.\eqref{Pop_tensor} using eqs.\eqref{sol_pn}, 
leading to
\begin{align}
    \mathcal{P}_{tt} & = 0 & \mathcal{P}_{rr} & = \alpha^2\beta^2\left(t\left(\alpha\beta + \gamma\right)\right)^{-\frac{2\gamma}{\alpha\beta + \gamma}},
\end{align}
however, unlike the Ricci tensor, the Poplwaski metric-torsion tensor has a trivial component in its temporal part, and, as a result of this, its a degenerate
...
Moreover, just like the Ricci tensor, its covariant derivative $\nabla \mathcal{P}$ does not vanishes, and, therefore there are metricity effects
coming from the torsion tensor $\mathcal{Q}^{\mathcal{P}}_{\mu\nu\lambda}$.
...
\end{align}
Using eq. \eqref{restriction_acceleration} along with eqs.\eqref{B1_a} \eqref{B2_a} allow us to define to intervals for the
