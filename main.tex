...
\end{aligned}
\end{equation}
while the non-trivial components of its vectorial part $\mathcal{A}$ is
\begin{equation}
    \label{A_ansatz}
    \A{t} = \eta(t).
\end{equation}
...
tensor are well behaved, then the factors $\left(\dot{g} + gh + 2\kappa\right)$ or $\left(-2\psi^2\right)$ play an analogue role to 
the scale factor $a^2(t)$.
