...
Einstein's theory of General Relativity, its a gravitational theory where the gravitational interactions are solely mediated by one 
fundamental field, the metric tensor \cite{Einstein_GR,Einstein_GR_Fundamentals,Einstein_GR_Bases}. Providing an ansatz compatible 
with the symmetries of the observable universe (cosmological principle), its possible to use Einstein's Field Equations \cite{Einstein_GR_Feqs} 
to describe the present universe.

With the introducing the cosmological (see Ref. \cite{Einstein_Cosmological_Constant}) constant its possible to obtain non-trivial dynamics to 
describe the evolution of the universe, leading to nine different scenarios, depending on the geometry factor $\kappa$ values (which can take 
the values of $-1$,$0$ and $1$), and if the cosmological constant its positive, negative or if it vanishes. Although, the introduction of a 
cosmological constant does indeed introduce dynamics to Einstein's static universe, its time evolution does not provide a precise description. 
In order to have a more accurate description of the observable universe, and to have non-trivial solutions, its required the introduction of the
energy-momentum tensor, along with the state equation. By taking into account this new information into Einstein's Field Equations, its possible 
to portray the observable Universe. Moreover, it allow us to clearly distinguish three different epochs, \textit{starting} from a radiation era, 
to a matter era and finally to an accelerated expansion due to dark matter presence. This is the universe describe by Friedmann–Lemaitre–Robertson–Walker 
(FRWL), see Refs. \cite{Frie,Friedmann,Lema,Lema_Expansion,Robertson_1,Robertson_2,Robertson_3}, and provides the foundations for the standard model of cosmology $\Lambda$CDM.

Although Einstein's formulation its the best theory we have so far to describe gravitational interactions, and there is an agreement between the theoretical
predictions with the observational data, to see a full review on its empirical evidence, please see Ref. \cite{Will_2014}. And recently, its more important success, is due to the
detection of gravitational waves by LIGO-Virgo collaborations \cite{Abbott_2016,Abbott_2017}. There are still non-trivial problems related to its description
of the geometry of space-time, specifically problems related to a quantization of gravity \cite{PhysRev.160.1113,PhysRev.162.1195,PhysRevD.10.401,PhysRevD.10.411} and the necessity of the dark sector to explain the galaxy rotation curve speed \cite{Rotation_Curve,Rotation_Curve_2}, along with accelerated expansion \cite{Riess_1998}.

These type of problems suggest, in principle, Einstein's theory of gravity must be an effective theory of another gravitational theory. In that sense, there are extensions of general relativity such as where the Einstein-Hillbert action its replaced by an arbitrary function, named $f(\mathcal{R})$, which was first proposed in Ref. \cite{10.1093/mnras/150.1.1}. An interesting case of this type of gravity, its the  Starobinsky's model \cite{STAROBINSKY198099} that allows to describe a universe without a singularity. Moreover, due to quantum corrections to general relativity, lead to an new contribution to the Einstein-Hillbert action due to curvature squared new term \cite{1979ZhPmR..30..719S}, which is a special case of $f(\mathcal{R})$ gravity. 

Because the choice on function $f(\mathcal{R})$ its an arbitrary one, there is an abundance of cosmological models, however, one could even further and at a fundamental level, claim, that the metric tensor and the affine connection (a prior) must be independent fields, these are the type of modify models known as \textit{metric-affine} models, and its formalism it is known as \textit{Palatini} formalism. There is an extensive collection on the literature related to metric-affine models \cite{Hehl_1995,Baldazzi_2022,Vitagliano_2011,Karahan_2012,SARDANASHVILY_2011}, and an extensive/complete review can be found in Ref.\cite{OLMO_2011}.
...
Sir Eddington \cite{Eddington1923-EDDTMT}, and pure affine formalism were proposed in Refs. \cite{Krasnov_2011,poplawski2007unified,POP_AWSKI_2007}. Even though, these are
affine models of gravity, strictly speaking, they are not purely affine, because they used metric square-root of the metric tensor, or the square-root of the Ricci tensor
to define the volume element.

There are other extensions to Einstein's theory of general relativity, for example Einstein-Cartan model \cite{ASENS_1924_3_41__1_0,ASENS_1925_3_42__17_0}, which uses the Einstein-Hillbert action to define gravitational interactions, with a non-symmetric connection by introducing non-trivial torsion effects. Moreover, the inclusion of extra dimensions where first proposed and studied by T. Kaluza and O. Klein in Refs. \cite{Kaluza_Klein}, in particular, uses a fifth dimension which allows to unify the gravitational with an electromagnetism forces. There are many gravitational models and within different classifications, an illustrative example of a so-called \textit{map of modify gravity} can be found in Ref. \cite{Shankaranarayanan_2022}.

In order to built a truly purely affine gravitational model of gravity, the metric tensor can not play any role in its formulation and in its building blocks, and this is
...
its features, and how its possible to still have two different metrics structures despite being a purely affine model. In Section \ref{sec:solutions} it is 
explore the cosmological solutions to the field equations using the \textit{logical scheme} strategy. In Section \ref{sec:analysis} we presented a detailed 
analysis and discussion of the cosmological solutions. Final remarks are presented in Section \ref{sec:final_remarks}. For the sake of completeness we include 
to two appendixes. In Appendix \ref{sec:Appendix_I} there is a more detailed description on how to build the cosmological ansatz for the irreducible fields
of the affine connection, using the Lie derivative. In Appendix \ref{sec:Appendix_II} we shown the explicit contribution to the field equation of each fundamental 
field.

\section{Polynomial Affine Gravity}
\label{sec:PAG}

The Polynomial Affine Gravity its a purely affine model of gravity whose fundamental field its the affine connection $\hat{\Gamma}$, whose 
irreducible fields can be decomposed into its symmetric and skew symmetric part
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
the first term correspond to the symmetric part of the connection $\hat{\Gamma}_{(\alpha}{}^{\beta}{}_{\gamma)} = \ctG{\alpha}{\beta}{\gamma}$, whereas
the last two terms are related to the torsion tensor, the former its the trace-less torsion $\B{\alpha}{\beta}{\gamma}$ and the latter correspond to its
vectorial part $\A{\alpha}$.

Its worth notice from eq.\eqref{affine_connection} that the symmetric part of the connection does not preserve the invariance under diffeomorphism, and therefore
can only go to the action indirectly through the covariant derivative $\nabla^{\Gamma}$. Additionally, the volume form its defined by the wedge product (which does not
required the metric tensor to be well-defined) as $\deV{\alpha\beta\gamma\delta}$, where $\deV{\alpha\beta\gamma\delta} = \mathrm{d}x^{\alpha}\wedge\mathrm{d}x^{\beta}\wedge
\mathrm{d}x^{\gamma}\wedge\mathrm{d}x^{\delta}$.

Therefore, the fundamental ingredients in our affine model of gravity are $\nabla$, $\mathcal{B}$, $\mathcal{A}$ and $\mathrm{d}V$. The action its build using
a sort of \textit{dimensional analysis} technique. The method allow us to build every single possible configuration of scalar densities objects using all the 
fundamental fields subject to geometrical constraint. For more information on these technique in four dimensions please refer to Refs.\cite{castillofelisola2016einsteins,Castillo_Felisola_2020}, 
and in three dimensions refer to Ref. \cite{Castillo_Felisola_2022_EPJC}.
...
\begin{equation}
    ...
    \end{split}
  \end{equation}
The action has twenty non trivial contributions coming from the fundamental fields, and the covariant derivative along with the curvature tensor are defined
with respect to the symmetric part of the connection, meaning $\nabla = \nabla^{\Gamma}$ and $\mathcal{R} = \mathcal{R}^{\Gamma}$. 

One of the most important features of this model its the lack of the metric tensor in its formulation, as a consequence of this, the number of terms
that can go to the action, its finite, this means that eq. \eqref{PAG_action} its the most general action one could possible build in this frame of work, we called
this property \textit{rigidity}. This does not happen in metric-affine model of gravity or modify gravities such as $f(R)$-theories, where there are no constraint
on the possible configurations of the Ricci scalar $\mathcal{R}$.

Additionally, even though, the manifold its endowed with an affine structure $\hat{\Gamma}$, it is still possible to have emergent metric structures. The first one
comes from the Ricci tensor $\mathcal{R}_{\mu\nu}$, which is defined by the contraction of the Riemann tensor.\footnote{The Riemann tensor its defined by the commutator of covariant derivatives 
acting on a vector, therefore, it does not required the existence of a metric structure.} 
\begin{equation}
    \label{chain}
    \hat{\Gamma} \to \nabla^{\Gamma} \to \Rie{\alpha\beta}{\gamma}{\delta} \to \Ri{\beta\delta},
\end{equation}
using the cosmological ansatz, describe in eq. \eqref{G_ansatz}, the components of the Ricci tensor are
\begin{align}
    \label{Ricci_tensor}
    \mathcal{R}_{tt} & = \dot{h} + h^2 & \mathcal{R}_{rr} & = \frac{\dot{g} + gh + 2\kappa}{1 - \kappa r^2},
\end{align}
where $\mathcal{R}_{\theta\theta}$ and $\mathcal{R}_{\varphi\varphi}$ are proportional to $\mathcal{R}_{rr}$, just like one would expect from a metric tensor. Notice,
if $\kappa = 0$, then the factor of $\dot{g} + gh$ resembles the scale factor $a^2(t)$ present in FRW cosmology.
...
\end{dmath}
under the symmetries described in eqs. eqs.\eqref{B_ansatz} and \eqref{A_ansatz}, Poplwaski's metric components are
\begin{align}
    \label{Pop_tensor}
    \mathcal{P}_{tt} & = \eta^2 & \mathcal{P}_{rr} & = -\frac{2\psi^2}{1 - \kappa r^2},
\end{align}
where $\mathcal{P}_{\theta\theta}$ and $\mathcal{P}_{\varphi\varphi}$ are proportional to $\mathcal{P}_{rr}$ just like a metric tensor. Just like before,
if $\kappa = 0$, then the factor $\psi^2(t)$ plays an analogue roles to the standard scale factor $a^2(t)$. Therefore, in our purely affine model still it is possible to define the notion of distance by two different objects. For more information, on how
the ansatz was build please refer to Appendix \ref{sec:Appendix_I}.

There are many more features, for example: i) all coupling constant are dimensionless,\footnote{This also points in the direction of a conformal symmetry, but this is speculative.} this its an indication that its power-counting renormalizable, which is a necessary
but no sufficient condition to guarantee that its renormalizable, ii) in the hypothetical scenario of a quantum version of Polynomial Affine Gravity, all its counter-terms
will have the form, of the ones already written in eq. \eqref{PAG_action}, because of its \textit{rigidity}, iii) in the torsion-free sector, its possible to recover
all Einstein's vacuum solutions.

Finally, the field equations are obtained by varying the action with respect each irreducible field using Kijowkski's formalism, see Ref. \cite{KJ_Formalism}, which we 
also reviewed in Ref. \cite{Castillo_Felisola_2020}. The complete set of the field equation are presented in Appendix \ref{sec:Appendix_II}, for each irreducible field of the affine connection.

\section{Cosmological solutions}
\label{sec:solutions}

Using the ansatz founded in eqs. \eqref{G_ansatz}, \eqref{B_ansatz} and \eqref{A_ansatz} the field equation are
\begin{dmath}
    \label{Feq_1}
...
Although the system of differential equations seems to be quite complicated, it can actually be solved analytically without any assumption. To see this,
we develop a \textit{logical scheme} the find its solutions. First, notice the structure of eqs. \eqref{Feq_1} and \eqref{Feq_2}, which can be 
written in a more subtle form
\begin{dmath}
    \label{Feq_1_1}
    \mathcal{F}(g,h,\psi,\eta)\psi = 0,
\end{dmath}
\begin{dmath}
    \label{Feq_2_2}
    \mathcal{G}(h,\psi,\eta)g = 0,
\end{dmath}
where $\mathcal{F}(g,h,\psi,\eta) = B_3\left(\dot{g} + gh + 2\kappa\right) - 2B_4\left(\dot{g} - gh\right) + 2D_6\eta g - 2F_3\psi^2$ and 
$\mathcal{G}(h,\psi,\eta) = B_3\eta\psi -2B_4\eta\psi + C_1\left(\dot{\psi} - 2h\psi\right)$. Thus, from eqs. \eqref{Feq_1_1} and \eqref{Feq_2_2}
its possible to distinguish four different branches:
\begin{align}
...
\end{align}
Obviously, the most interesting branch its eq. \eqref{branch_1}, which is the most general case, and the simplest branch will be 
eq.\eqref{branch_4}.
...
\end{equation}
It is worth to notice the above non-trivial solution it is valid only for the case $\kappa \neq 0$ this can bee seen directly from eqs. \eqref{Feq_B2_1}
\eqref{B2_k_constraint_1} and \eqref{B2_k_constraint_2}. 

If one, impose the constraint $\kappa = 0$, then from eq. \eqref{Feq_B2_1} tell us that $\psi(t) = 0$ which solves entirely the other
equations. Therefore, the other functions remain unknown.

\subsection{Third branch}

With the constraint $\psi(t)=0$, impose a strong restriction on the systems of differential equations, reducing its complexity to just
...
\end{equation}
the above differential equation has three unknown functions of time $h(t)$, $g(t)$ and $\eta(t)$ which can not be solved. The system for this branch
undetermined.

\subsection{Fourth branch}

The fourth branch, impose strong restriction on the field equations by the requirements $g(t) = 0$ and $\psi (t) = 0$, the field equations are reduced
to just one algebraic equation
\begin{equation}
    \kappa\left(hC_1 + B_3\eta\right) = 0
\end{equation}
the above equation can be solved by setting $\kappa = 0$ then the functions $h(t)$ and $\eta(t)$ are undetermined, or by the relation $h(t) = -\eta(t)\left(\frac{B_3}{C_1}\right)$
where $\eta(t)$ its an arbitrary function.

\section{Analysis of the solutions}
\label{sec:analysis}

The only non-trivial solutions can be found in branch \eqref{branch_1}, thus, this is the only branch which will be studied in the following subsection.
Because of the unknown initial condition in standard cosmology, in order to simply the calculation and without any loss of 
generalization, we are going to assume $\phi_0 \to 1$, $\phi_1 \to 0$ and $g_0 \to 0$. Thus, the affine functions are simplify 
\begin{align}
...
\end{align}
where $h(t)$ and $g(t)$ defines the symmetric part of the connection, whereas $\psi(t)$ and $\eta(t)$ defines the anti symmetric part.

\subsection{Emergent metrics}

As mentioned in Sec. \ref{sec:Introduction} it is possible to obtain emergent metric structures in this purely affine model of gravity. The first metric tensor,
comes from the Ricci tensor, using eqs. \eqref{sol_hg} in eqs. \eqref{Ricci_tensor} we compute the Ricci tensor
\begin{align}
...
\end{equation}
It is worth to recall that the Ricci's tensor spatial component plays an analogue role to the scale factor in the FRW universe, thus
\begin{equation}
    \Sigma\left(\alpha,\beta,\gamma\right)t^{2\left(\frac{\alpha\beta - \alpha\beta -\gamma}{\alpha\beta + \gamma}\right)} \longleftrightarrow a^2(t).
\end{equation}
Therefore, we can use eq.\eqref{R_spatial} to define the notion of distance, as long as its well behaved, however, unlike in general relativity, the metriciy condition
its not satisfied in the affine geometry, meaning $\nabla \mathcal{R}$ does not vanishes, and therefore it does not preserve parallelism.

In principle, it is also possible to compute Poplwaski's metric-torsion tensor which was defined in eq.\eqref{Pop_tensor} using eqs. \eqref{sol_pn}
\begin{align}
    \mathcal{P}_{tt} & = 0 & \mathcal{P}_{rr} & = \alpha^2\beta^2\left(t\left(\alpha\beta + \gamma\right)\right)^{-\frac{2\gamma}{\alpha\beta + \gamma}},
\end{align}
however, unlike the Ricci tensor, the Poplwaski metric-torsion tensor has a trivial component in its temporal part, as a consquence of this, its a degenerate
tensor which can not be invertible, therefore, it can not act as a well behaved metric tensor, and it is not possible to used to the define the notion of distance. 
Additionally, just like the Ricci tensor, its covariant derivative $\nabla \mathcal{P}$ does not vanishes, and therefore, does not preserved the notion
of parallelism.
...
Having a well defined affine scale factor coming from the Ricci tensor as in eq. \eqref{R_spatial}, we can see that this is a solution \textit{sort-off} like a
...
\end{equation}
, meaning we could use the torsion tensor to have different types of affine scales factor. For example, fixing the $\gamma = -\frac{\alpha\beta}{3}$, turns the affine scale factor
\begin{equation}
    a_f(t) = a_0 t^{1/2},
\end{equation}
...
\end{equation}
\begin{figure}[ht]
    \includegraphics[scale=0.4]{Affine_Radiation}
    \caption{Radiation era in PAG due to torsion effects}
    \label{Affine_Radiation_plot}
\end{figure}
This type of solution allow us to emulate the radiation era. The affine radiation era can be plotted for different parameter's values which 
satisfy the constraint condition, see Fig. \ref{Affine_Radiation_plot}.


In the same spirit, we can fix $\gamma = -\frac{2\alpha\beta}{5}$ which leads to
\begin{equation}
    a_f(t) = a_{0}t^{2/3},
\end{equation}
where we use $a_0$ as 
\begin{equation}
    a_0 = \sqrt{\frac{-24}{25}\left(\frac{3}{5}\right)^{/13}\alpha^{13/3}\beta^{10/3}},
\end{equation}    
which, in order to be real we need to impose the restriction $\beta <0$, meaning, we use the same constraint written in eq.\eqref{constraint}. This type
of solution allow us to emulate a non-relativistic matter era.
\begin{figure}[ht]
    \includegraphics[scale=0.4]{Affine_Matter}
...
If we want to have an accelerated expansion due to torsion effects, we need to impose
\begin{equation}
...
\end{equation}
First, wee need to impose 
\begin{equation}
    \frac{\alpha\beta}{\alpha\beta + \gamma} - 3    > 0 
\end{equation}
which leads to
\begin{equation}
...
\end{equation}
this conditions ensures that the time evolution will be defined positive. Additionally, we have two branches, each with two conditions
\begin{align}
    B1: \alpha\beta & > -2\gamma & \gamma & > 0 & B2: \alpha\beta & < -2\gamma & \gamma & < 0 
\end{align}
both branches along with eq.\eqref{restriction_acceleration} ensures an acceleration expansion.  
\begin{figure}[ht]
    \includegraphics[scale=0.4]{Affine_Expansion}
...
In this work, we have reviewed the Polynomial Affine Gravity model with torsion effects. The inclusion of the non-vanishing torsion tensor,
allows for an overdetermined system of differential equations, see eqs. \eqref{Feq_1} to \eqref{Feq_5}, whereas without torsion effects, the field equations are reduced
to the \textit{harmonic curvature} $\nabla_\alpha \mathcal{R}_{\beta\gamma}{}^{\alpha}{}_{\delta} = 0$, which through Bianchi's identity 
...
the system is undetermined and, in order to extract information, its necessary to provide with an ansatz for $h(t)$ or $g(t)$. A priori, there is no physical
reason to fix one of the functions, however, one could explore subspace of this torsion-free solutions, by studying $\nabla_\alpha \mathcal{R}_{\beta\gamma} = 0$.
This, leads to a three differential equations, which can be solved analytically and provides with an exact solution for $h(t)$ and $g(t)$ functions. Even more, the Ricci tensor
its well behaved and it can be used as a metric tensor, moreover, this type of solution its known as de-Sitter/Anti de-Sitter solution, where the cosmological constant
its an integration constant. For more information on solutions in the torsion-free sector, refer to Refs. \cite{Castillo_Felisola_2020}.

By the introduction of torsion effects with the tensors $\mathcal{B}$ and $\mathcal{A}$, the dynamical equations introduce non-trivial effects. Because of the presence
of the torsion tensor, now, there are two emergent metric structures. This could be an analogue situation to the FRW universe, in principle in its universe, there is the 
Ricci tensor (in Einstein's Field Equations) and there is also the energy-momentum tensor, which can act as an emergent metric tensor when its no degenerate. In that sense,
the Ricci tensor affine could play role of the geometrical distance (providing an affine scale factor), whereas the Poplawski metric-torsion tensor plays the role of a matter tensor, or vice-versa.

Interestingly, the system of field equations eqs. \eqref{Feq_1} to \eqref{Feq_5} present four different branches of solution, of which only one can be solved exactly. This solution,
its found for the traceless part of the torsion $\psi(t)$ and it resembles the energy density from general relativity. Because of that, we can study different configurations
for the $\alpha$, $\beta$ and $\gamma$ parameters, and analyze their effects in the metric tensor coming from the Ricci. For specific values of those parameters, we are able
to emulate the radiation era, the non-relativistic matter era. Additionally, we are able to define constraint for those parameters in order to secure an accelerated expansion, those constraint enable us to emulate dark matter effects.

As we see, the introduction of torsion effects in this affine model, allow us to emulate matter effects in the emergent metric structure coming from the Ricci tensor. In this formulation, the three major eras, are due to the presence of torsion and not an energy-momentum tensor. 

\section{Acknowledgments}

We are specially thankful to the developers of the software SageMath \cite{sagemath} and SageManifolds \cite{Gourgoulhon_2015,Gourgoulhon_2018}, along
with the developers of the Cadabra software \cite{peeters2018introducing,Peeters2018,Peeters_2007}, both softwares were used extensively in our calculations.
Specifically, for computing the Lie derivative of the irreducible fields of the affine connection along the Killing vectors, and the variation
of each term in the Polynomial Affine Gravity's action, respectively.

\appendix

\section{Appendix I: Cosmological ansatz}
\label{sec:Appendix_I}

The symmetries of the universe at large scale are given by the cosmological principle: isotropy and homogeneity, those symmetries can explicitly by 
written as Killing vectors
\begin{align}
    \label{killing_vectors}
    J_1 & = (0 , 0, -\cos \varphi, \cot\theta\sin\varphi) \\
    J_2 & = (0 , 0, \sin \varphi, \cot\theta\cos\varphi) \\
    J_3 & = (0 , 0, 0, 1) \\
    P_1 & = \sqrt{1 - \kappa r^2}\left(0 , \cos\varphi\sin\theta, \frac{\cos\varphi\cos\theta}{r}, -\frac{\sin\varphi}{r\sin\theta}\right) \\
    P_2 & = \sqrt{1 - \kappa r^2}\left(0 , \sin\varphi\sin\theta, \frac{\sin\varphi\cos\theta}{r}, \frac{\cos\varphi}{r\sin\theta}\right) \\
    P_3 & = \sqrt{1 - \kappa r^2}\left(0 , \cos\theta, -\frac{\sin\theta}{r}, 0\right) 
\end{align}
where $J_i$ are the generators of the rotation symmetry (isotropy) and $P_i$ are the generators of the translational symmetry (homogeneity). In order to 
build an ansatz compatible with the above symmetries, we use the Lie derivative along those generators.

The lie derivative of the connection $\Gamma$ its given by
\begin{equation}
    \label{Lie_aff_connection}
    \mathcal{L}_{\xi}\ctG{\beta}{\alpha}{\gamma} = \xi^{\mu}\partial_{\mu}\ctG{\beta}{\alpha}{\gamma} - \ctG{\beta}{\mu}{\gamma}\partial_{\mu}\xi^{\alpha}
    + \ctG{\mu}{\alpha}{\gamma}\partial_{\beta}\xi^{\mu} + \ctG{\beta}{\alpha}{\mu}\partial_{\gamma}\xi^{\mu} + \frac{\partial^2\xi^{\alpha}}{\partial x^{\beta}\partial x^{\gamma}},
\end{equation}
where the last term appears because the affine connection under a coordinate transformations has a non homogeneous term.  The computations of the Lie 
derivative eq.\eqref{Lie_aff_connection} along the Killing vectors in eq. \eqref{killing_vectors} leads to
\begin{align}
    \label{G_ansatz}
...
The Lie derivative for the $\mathcal{B}$ field is
\begin{dmath}
    \label{Lie_B}
    \mathcal{L}_{\xi}\B{\beta}{\alpha}{\gamma} = \xi^{\mu}\partial_{\mu}\B{\beta}{\alpha}{\gamma} - \B{\beta}{\mu}{\gamma}\partial_{\mu}\xi^{\alpha}
    + \B{\mu}{\alpha}{\gamma}\partial_{\beta}\xi^{\mu} + \B{\beta}{\alpha}{\mu}\partial_{\gamma}\xi^{\mu},
\end{dmath}
performing the above calculation of eq.\eqref{Lie_B} along the Killing vectors \eqref{killing_vectors} give the number of independent 
...
\end{aligned}
\end{equation}
Notice the trace-less part of the torsion tensor has only one time-dependent function to defined the tensor completly.

The last part of the ansatz, its given by $\mathcal{A}$ field. Its Lie derivative is 
\begin{dmath}
    \label{Lie_A}
    \mathcal{L}_{\xi}\A{\alpha} = \xi^{\mu}\partial_{\mu}{\alpha} + \A{\mu}\partial_{\alpha}\xi^{\mu},
\end{dmath}
and the calculation of eq.\eqref{Lie_A} along the Killing vectors in eq.\eqref{killing_vectors} gives 
\begin{equation}
    \label{A_ansatz}
    \A{t} = \eta(t).
\end{equation}
...
\section{Appendix II: Field equation}
