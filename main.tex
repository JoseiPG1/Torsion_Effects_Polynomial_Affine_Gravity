...
recover Einsteins's field equations coupled with a scalar field, see Ref.\cite{castillofelisola2023inflationary}.
...
Nonetheless, we shall show that, it can actually be solved analytically without any assumption. For this end,
...
\end{dmath}
The above equation can be solved for the special case $\kappa = 0$, with the variable change $\psi (t) = \dot{\phi}(t)$, 
...
\end{equation}
From the above expression and using the relations in eqs. \eqref{B1_eta}, \eqref{B1_h} and \eqref{B1_g} it is direct to find the rest of the
affine functions
\begin{align}
    \eta(t) & = 0,\\
...
\end{equation} 
The constraint for the coupling constants, turns eq. \eqref{Feq_B2_4} into a first order differential equation for $\eta(t)$
\begin{equation}
...
\end{equation}
It is worth to notice the above nontrivial solution it is valid only for the special case where $\kappa \neq 0$, 
...
As mentioned in Sec. \ref{sec:PAG} it is possible to obtain emergent metric structures 
in this purely affine model of gravity. The fist type of metric like tensor comes from the Ricci
...
\end{equation}
It is worth to recall that the spatial component of the Ricci tensor plays an analogue role 
to the scale factor in the FLRW universe, thus
\begin{equation}
...
\subsection{Matter emulation from torsion fields}

Having a well defined affine descendent scale factor coming from the Ricci tensor, defined in eq.\eqref{R_spatial}, is a power-law 
solution depending of the coupling constant $\alpha$, $\beta$ and $\gamma$, which are present due to the torsion field. In order 
to give a physical meaning, we can inquire the value of these parameters that possess a well-known behaviour in G.R., specifically
in FLRW solutions.

Note that $\psi(t)$ has a similar form to the energy density expression in the standard model of cosmology
\begin{equation}
...
constant $\alpha\beta$, the former are $\alpha\beta \in \left[-2\gamma, -\frac{3}{2}\gamma\right]$, 
whereas the latter is given by $\alpha\beta <-2\gamma$. 
...
is no physical reason to fix one of the functions, however, one could explore subspace of 
this torsion-free solutions, by studying  integrability conditions such as $\nabla_\alpha 
\mathcal{R}_{\beta\gamma} = 0$. This, leads to a three differential equations, which can 
...
effects in the emergent metric structure coming from the Ricci tensor. In this formulation, the three major 
eras, are  due to the presence of torsion and not an energy-momentum tensor. In that sense, we could 
argue that, the accelerated expansion of the universe could be explain by torsion fields, which is a 
geometrical object.

Additionally, it is possible to explore others types of configurations for the parameters
$\alpha$, $\beta$ and $\gamma$, providing different types of scale factors, which usually, can be obtained
in General Relativity by the introduction of a scalar field and its potential energy term. This opens 
the door for new possibles ways of explaining the origin and the nature of the dark sector.
