...
One of the most important features of this model its the lack of the metric tensor 
...
it is still possible to have emergent metric structures. The first one
comes from the Ricci tensor $\mathcal{R}_{\mu\nu}$, which is defined by the contraction of 
the Riemann tensor.\footnote{The Riemann tensor its defined by the commutator of covariant 
derivatives acting on a vector, therefore, it does not required the existence of a metric structure.} 
\begin{equation}
    \label{chain}
...
is a necessary condition but no sufficient condition to guarantee that the Polynomial Affine Gravity is renormalizable. Moreover,
it suggest a conformal symmetry.
...
while the potential energy re-scale the volume element. Just like before, we can recover general relativity and the Klein-
Gordon field equations, see Ref.[XX]. 

The field equations are obtained by varying the action with respect to each irreducible field using Kijowkski's 
